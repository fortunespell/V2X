...
public:
  ...
  // Function to print the received messages from the master and handle the distance data
  void onReceive(const uint8_t *data, size_t len, bool broadcast) {
    Serial.printf("Received a message from master " MACSTR " (%s)\n", MAC2STR(addr()), broadcast ? "broadcast" : "unicast");
    Serial.printf("  Message: %s\n", (char *)data);
    
    // Convert received data to integer distance
    long distance = atol((char *)data);
    Serial.printf("  Distance: %ld cm\n", distance);

    // Blink LED
    digitalWrite(LED_PIN, HIGH);
    delay(500); // Adjust the blink duration as needed
...
void register_new_master(const esp_now_recv_info_t *info, const uint8_t *data, int len, void *arg) {
  if (memcmp(info->des_addr, ESP_NOW.BROADCAST_ADDR, 6) == 0) {
    ...
    masters.back().onReceive((const uint8_t *)data, len, true);
  } else {
...
void setup() {
  ...
  // Initialize LED pin and buzzer pin
